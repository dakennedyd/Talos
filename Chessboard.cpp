...
Bitboard Chessboard::attackRays[65][8];
...
Bitboard Chessboard::checkIfenPassantPossibleOnNextMove(const Square a, const Square b)
{
	...
	if ((startSquare & mBoard[WHITE_PAWNS_BOARD]) && (startSquare & RANK2) && (endSquare & RANK4))
	{
		return startSquare >> 8;
	}

	if ((startSquare & mBoard[BLACK_PAWNS_BOARD]) && (startSquare & RANK7) && (endSquare & RANK5))
	{
		return startSquare << 8;
	}
...
Piece Chessboard::getPieceFromSquare(const Square square)
{
	...
	std::cout << "info command error: no piece on square\n";
	exit(1);
...
void Chessboard::setState(const std::string &FENstring)
{
	...
	for (unsigned char c : state)
	{
		...
		if (std::isdigit(c) == 0) //not numeric
		{
			...
			{
			case 'p':
				setPieceInBoard(bitPosition, mBoard[BLACK_PAWNS_BOARD]);
				break;
			case 'r':
				setPieceInBoard(bitPosition, mBoard[BLACK_ROOKS_BOARD]);
				break;
			case 'n':
				setPieceInBoard(bitPosition, mBoard[BLACK_KNIGHTS_BOARD]);
				break;
			case 'b':
				setPieceInBoard(bitPosition, mBoard[BLACK_BISHOPS_BOARD]);
				break;
			case 'q':
				setPieceInBoard(bitPosition, mBoard[BLACK_QUEEN_BOARD]);
				break;
			case 'k':
				setPieceInBoard(bitPosition, mBoard[BLACK_KING_BOARD]);
				break;
			case 'P':
				setPieceInBoard(bitPosition, mBoard[WHITE_PAWNS_BOARD]);
				break;
			case 'R':
				setPieceInBoard(bitPosition, mBoard[WHITE_ROOKS_BOARD]);
				break;
			case 'N':
				setPieceInBoard(bitPosition, mBoard[WHITE_KNIGHTS_BOARD]);
				break;
			case 'B':
				setPieceInBoard(bitPosition, mBoard[WHITE_BISHOPS_BOARD]);
				break;
			case 'Q':
				setPieceInBoard(bitPosition, mBoard[WHITE_QUEEN_BOARD]);
				break;
			case 'K':
				setPieceInBoard(bitPosition, mBoard[WHITE_KING_BOARD]);
				break;
			}
			bitPosition++;
		}
...
void Chessboard::printBitboard(const Bitboard bitboard)
{
	...
	for (uint64_t i = 0; i < 8; i++)
	{
		...
		{
			b = (static_cast<uint64_t>(1) << (i * 8 + j)) & bitboard;
			if (b != 0)
				std::cout << " X ";
			else
				std::cout << " - ";
...
void Chessboard::printBoard()
{
	...
	for (uint64_t i = 0; i < 8; i++)
	{
		...
		for (uint64_t j = 0; j < 8; j++)
		{
			b = (uint64_t(1) << (i * 8 + j)) & mBoard[BLACK_PAWNS_BOARD];
			if (b != 0)
			{
				std::cout << " p ";
				continue;
			}
			b = (uint64_t(1) << (i * 8 + j)) & mBoard[BLACK_ROOKS_BOARD];
			if (b != 0)
			{
				std::cout << " r ";
				continue;
			}
			b = (uint64_t(1) << (i * 8 + j)) & mBoard[BLACK_KNIGHTS_BOARD];
			if (b != 0)
			{
				std::cout << " n ";
				continue;
			}
			b = (uint64_t(1) << (i * 8 + j)) & mBoard[BLACK_BISHOPS_BOARD];
			if (b != 0)
			{
				std::cout << " b ";
				continue;
			}
			b = (uint64_t(1) << (i * 8 + j)) & mBoard[BLACK_QUEEN_BOARD];
			if (b != 0)
			{
				std::cout << " q ";
				continue;
			}
			b = (uint64_t(1) << (i * 8 + j)) & mBoard[BLACK_KING_BOARD];
			if (b != 0)
			{
				std::cout << " k ";
				continue;
			}
			b = (uint64_t(1) << (i * 8 + j)) & mBoard[WHITE_PAWNS_BOARD];
			if (b != 0)
			{
				std::cout << " P ";
				continue;
			}
			b = (uint64_t(1) << (i * 8 + j)) & mBoard[WHITE_ROOKS_BOARD];
			if (b != 0)
			{
				std::cout << " R ";
				continue;
			}
			b = (uint64_t(1) << (i * 8 + j)) & mBoard[WHITE_KNIGHTS_BOARD];
			if (b != 0)
			{
				std::cout << " N ";
				continue;
			}
			b = (uint64_t(1) << (i * 8 + j)) & mBoard[WHITE_BISHOPS_BOARD];
			if (b != 0)
			{
				std::cout << " B ";
				continue;
			}
			b = (uint64_t(1) << (i * 8 + j)) & mBoard[WHITE_QUEEN_BOARD];
			if (b != 0)
			{
				std::cout << " Q ";
				continue;
			}
			b = (uint64_t(1) << (i * 8 + j)) & mBoard[WHITE_KING_BOARD];
			if (b != 0)
			{
				std::cout << " K ";
				continue;
			}
			b = (uint64_t(1) << (i * 8 + j)) & mBoard[ALL_PIECES_BOARD];
			if (b == 0)
				std::cout << " - ";
			else
				std::cout << " ? ";
	...
	//printBitboads();
	std::string stm;
	if (mPlayerToMove == WHITE)
		stm = "White";
	else
		stm = "Black";
	std::cout << "Side to move:" << stm;
	std::cout << "\ncastling available K:" << mCastlingAvailable[1] << "  Q:" << mCastlingAvailable[3] << "  k:" << mCastlingAvailable[2] << "  q:" << mCastlingAvailable[4] << "\n";
}
...
void Chessboard::makeMove(const Move &move)
{
	...
	if (move.mPromoteTo == Piece::NO_PIECE)
	{
	...
	{
		mCastlingAvailable[1 + move.mSide] = 0;
		mCastlingAvailable[3 + move.mSide] = 0;
...
void Chessboard::unmakeMove()
{
	...
	if (move.mCaptured != Piece::NO_PIECE) 								//if move is a capture
	{
		if (move.enPassantCapture)
		{
			...
			mBoard[WHITE_PIECES_BOARD + otherSide] |= CapturedPawnPosition[otherSide];	//restore the captured pawn to the color bitboard
			mBoard[WHITE_PAWNS_BOARD + otherSide] |= CapturedPawnPosition[otherSide];	//restore the captured pawn to the pawn bitboard
			mBoard[ENPASSANT_BOARD] = CapturedPawnPosition[otherSide];
			mBoard[ALL_PIECES_BOARD] |= CapturedPawnPosition[otherSide];				//restore captured pawn to ALL_PIECES_BOARD
		}
		else
		{
			mBoard[WHITE_PIECES_BOARD + otherSide] |= moveToSquare;						//restore the captured piece to the color bitboard
			mBoard[move.mCaptured + otherSide] |= moveToSquare;							//restore the captured piece to the piece bitboard
		}
	}
	else
	{
	...
	if (move.mCastle == Castling::KINGSIDE)
	{
		...
		mBoard[WHITE_PIECES_BOARD + move.mSide] |= (rook << 2);
		mBoard[WHITE_PIECES_BOARD + move.mSide] &= ~rook;
		mCastlingAvailable[1 + move.mSide] = 1;
		mCastlingAvailable[3 + move.mSide] = 1;
	}
	if (move.mCastle == Castling::QUEENSIDE)
	{
		Bitboard rookColor[2] = {RANK1, RANK8};
...
bool Chessboard::isKingOnCheck(const Player player)
{
	if(player == WHITE)
	{
		return checkIfSquaresAreAttackedByBlack(mBoard[WHITE_KING_BOARD]);
	}
	else{
		return checkIfSquaresAreAttackedByWhite(mBoard[BLACK_KING_BOARD]);
	}
}
...
void Chessboard::whiteKingMoves()
{
	...
	{
		Bitboard squares = 8070450532247928832ULL;
	...
	{
		Bitboard squares = 2161727821137838080ULL;
		if (((squares & ~kingBoard) & mBoard[ALL_PIECES_BOARD]) == 0) //if squares are not occupied
		{
...
std::vector<Move> Chessboard::generateMovesPerft()
{
	mPossibleMoves.clear();
	if (mPlayerToMove == Player::WHITE)
	{
